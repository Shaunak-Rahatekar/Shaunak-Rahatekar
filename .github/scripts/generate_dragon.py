...
def generate_svg(calendar_data):
    ...
    <style>
        ...
        @keyframes burn {{
            ...
            10% {{ fill: #ff9900; transform: scale(1.2); opacity: 1; }}
            20% {{ fill: #ff4d4d; transform: scale(0.8); opacity: 0.8; }}
            30% {{ fill: #1a1a1a; transform: scale(0.2); opacity: 0.2; }}
        ...
        @keyframes fly {{
            0% {{ transform: translate(-100px, 20px) scale(0.5); }}
            {fly_duration / anim_duration * 100}% {{ transform: translate({svg_width + 50}px, 0px) scale(0.5); }}
            100% {{ transform: translate({svg_width + 50}px, 0px) scale(0.5); }}
        }}
        .fire {{
        ...
        @keyframes breathe {{
            0% {{ opacity: 0; transform: translate(-100px, 20px) scale(0.5); }}
            5% {{ opacity: 0.8; transform: translate(0px, 20px) scale(0.5); }}
            {fly_duration / anim_duration * 100}% {{ opacity: 0.8; transform: translate({svg_width + 50}px, 0px) scale(0.5); }}
    ...
    for i, week in enumerate(weeks):
        x = offset_x + i * step
        delay = (x / svg_width) * fly_duration
        for j, day in enumerate(week['contributionDays']):
            y = offset_y + j * step
            count = day['contributionCount']
            color = get_color(count)
            # Only animate if there's a contribution, or animate all of them? 
            # Animating all makes it look like the whole grid is destroyed
            classes = "contrib grid"
            
            svg += f'        <rect x="{x}" y="{y}" width="{square_size}" height="{square_size}" rx="2" class="{classes}" style="--base-color: {color}; fill: {color}; animation-delay: {delay}s;" />\n'
            
    svg += '    </g>\n'
    
    # Dragon silhouette (Simplified)
    dragon_path = "M 30,50 Q 50,30 70,50 Q 80,40 100,60 Q 90,80 70,70 Q 50,90 30,70 Q 20,80 10,60 Q 20,40 30,50 Z M 70,50 L 90,30 L 80,40 Z M 30,50 L 10,30 L 20,40 Z"
    # A better dragon/beast shape:
    dragon_path = "M46.7,26.4c0,0-15.5-8.5-22.5-3.5c-7,5,0,26.1,0,26.1s-10-8.8-11.5-6.8c-1.5,2,4,21.6,4,21.6s-18.1-15.1-18.1-12c0,3.1,16.6,26.6,16.6,26.6s-18.6-1.5-17.6,1.5c1,3,27.1,19.1,27.1,19.1s-15.1,14.6-12.6,16.6c2.5,2,19.6-12.6,19.6-12.6s2.5,24.1,5.5,23.1c3-1,5-24.1,5-24.1s14.6,16.6,17.6,14.6c3-2-8.5-23.1-8.5-23.1s27.1,8.5,28.1,5.5c1-3-12.6-19.1-12.6-19.1s24.1-3.5,23.1-6.5c-1-3-22.1-1.5-22.1-1.5s15.6-21.6,13.6-24.1C80.3,45,61.7,59.5,61.7,59.5S66.3,37.4,63.2,36C60.2,34.5,46.7,26.4,46.7,26.4z"
    
    # We will use a group for the dragon to scale it properly
    svg += f'''
    <g class="fire">
        <path d="M 90,60 Q 150,40 200,60 Q 150,80 90,60 Z" fill="#ff9900" opacity="0.6"/>
        <path d="M 90,60 Q 130,50 160,60 Q 130,70 90,60 Z" fill="#ffcc00" opacity="0.8"/>
    </g>
    
    <g class="dragon">
        <path d="{dragon_path}" fill="#ff4d4d" transform="translate(0, 0) scale(1.5)" />
    </g>
    '''
...
def main():
    ...
    with open("dist/github-dragon.svg", "w") as f:
        f.write(svg_content)
